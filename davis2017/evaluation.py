...
class DAVISEvaluation(object):
    ...
    @staticmethod
    def _evaluate_semisupervised(all_gt_masks, all_res_masks, metric):
        if all_res_masks.shape[0] != all_gt_masks.shape[0]:
            sys.stdout.write("\nIn your PNG files there is an index higher than the number of objects in the sequence!")
            sys.exit()
        j_metrics_res, f_metrics_res = np.zeros(all_gt_masks.shape[:2]), np.zeros(all_gt_masks.shape[:2])
        for ii in range(all_gt_masks.shape[0]):
            void_masks = all_gt_masks[ii, ...] == _VOID_LABEL
            if 'J' in metric:
                j_metrics_res[ii, :] = db_eval_iou(all_gt_masks[ii, ...], all_res_masks[ii, ...], void_masks)
            if 'F' in metric:
                f_metrics_res[ii, :] = db_eval_boundary(all_gt_masks[ii, ...], all_res_masks[ii, ...], void_masks)
        return j_metrics_res, f_metrics_res

    @staticmethod
    def _evaluate_unsupervised(all_gt_masks, all_res_masks, metric, max_n_proposals=20):
        ...
        for ii in range(all_gt_masks.shape[0]):
            for jj in range(all_res_masks.shape[0]):
                void_masks = all_gt_masks[ii, ...] == _VOID_LABEL
                if 'J' in metric:
                    j_metrics_res[jj, ii, :] = db_eval_iou(all_gt_masks[ii, ...], all_res_masks[jj, ...], void_masks)
                if 'F' in metric:
                    f_metrics_res[jj, ii, :] = db_eval_boundary(all_gt_masks[ii, ...], all_res_masks[jj, ...], void_masks)
    ...
    def evaluate(self, res_path, metric=('J', 'F'), debug=False):
        ...
        for seq in tqdm(list(self.dataset.get_sequences())):
            all_gt_masks, all_masks_id = self.dataset.get_all_masks(seq, True)
            ...
            if self.task == 'unsupervised':
                ...
                j_metrics_res, f_metrics_res = self._evaluate_semisupervised(all_gt_masks, all_res_masks, metric)
            for ii in range(all_gt_masks.shape[0]):
